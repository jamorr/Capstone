...
def get_connection(
    api_key: str | pathlib.Path = pathlib.Path(__file__).parents[2] / "api_key.txt",
):
    if (isinstance(api_key, str) and os.path.exists(api_key)) or isinstance(
        api_key, pathlib.Path
    ):
        try:
...
def get_311_data(
    ...
    numeric_feats = ["latitude", "longitude"]
    for f in time_feats:
        # if df[f].dtype
        try:
...
def get_crime_data(
    ...
    try:
        date_time = {
            "crime_close_date": ["crime_close_date", "crime_close_time"],
            "crime_date": ["crime_date", "crime_time"],
...
def check_autocorr_ts(target_df: pd.DataFrame, lags: int = 24, plots: bool = True):
    ...
    for c in target_df.columns:
        ...
        print(
            "Best Lag:",
            max(list(acorr_ljungbox(target_df[c], lags=lags, auto_lag=True).index)),
        )
        if plots:
